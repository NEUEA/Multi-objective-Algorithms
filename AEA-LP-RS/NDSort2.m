function [FrontNo,MaxFNo,PP] = NDSort2(varargin)
%NDSort2 - Do non-dominated sorting by efficient non-dominated sort (ENS)
%
...
%--------------------------------------------------------------------------
    ...
    FrontNo        = inf(1,N);
    MaxFNo         = 0;
    Num_D          = cell(1,N); %The first row represents the domination of other quantities 
    Be_dominated   = zeros(1,N);
            ...
            if FrontNo(i) == inf
                ...
                for j = i-1 : -1 : 1
                    ...
                    if FrontNo(j) == MaxFNo
                        ...
                        Dominated = m > M;
                        if Dominated || M == 2                            
                            Be_dominated(1,i) =  Be_dominated(1,i)+1;
                            Num_D{1,j} = [Num_D{1,j},i];
                            k=k+1;
                        end
                        if k>=1
                            Dominated = true;
    ...
    PP = zeros(1,N);
    for i = 1:N
        if ~isempty(Num_D{1,i})
            for j = 1:length(Num_D{1,i})
                PP(1,i) = PP(1,i) + 1/Be_dominated(1,Num_D{1,i}(j));
            end
        end
    end
